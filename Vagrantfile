...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "security", type: "shell" do |s|
    s.name = "security"
    s.path = "provisionSecurity.bash"
  end

  config.vm.provision "packages", type: "shell" do |s|
    s.name = "packages"
    s.path = "provisionPackages.bash"
  end

  config.vm.provision "devtools", type: "shell" do |s|
    s.name = "devtools"
    s.path = "provisionDevTools.bash"
    s.privileged = false
  end
